        ...
        RuleForEach(x => x.Images)
             .ChildRules(image =>
             {
                 image.RuleFor(file => file.Length)
                     ...
                     .WithMessage((fileInstance, lengthProperty) =>
                         $"Image '{fileInstance.FileName}' exceeds the 5MB size limit.");

                 image.RuleFor(file => file.ContentType)
                     .Must(contentType => contentType == "image/jpeg" ||
                                         contentType == "image/png" ||
                                         contentType == "image/webp")
                     .WithMessage((fileInstance, contentTypeProperty) =>
                         $"Image '{fileInstance.FileName}' must be a JPG, PNG, or WEBP format.");
             });
    }
}