...
public class StripeWebhookHandler
{
    ...
    public StripeWebhookHandler(AppDbContext db)
    {
        _db = db;
    }
        ...
        CancellationToken ct)
    {
        ...
        var cartItems = JsonSerializer.Deserialize<List<CartItemSnapshot>>(session.Metadata["cart"], JsonOptions)
            ?? [];
        ...
        for (var i = 0; i < lineItemsList.Count && i < cartItems.Count; i++)
        {
            ...
            if (cartItem.ProductVariantId is Guid variantId)
            {
